          ...
          {filtered.map(t => (
            <div key={t.id} className="card" style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 13, marginBottom: 14 }}>
                ...
                <div
                  title={formatAOAFull(t.orcamento_total)}
                  style={{ fontWeight: 700, color: 'var(--color-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                ><Icon.Dollar size={14} style={{ display: 'inline', verticalAlign: 'middle', marginRight: 6 }}/> {formatarOrcamento(t.orcamento_total)}</div>
              </div>
